...
const findLessonByRef = async (lessonRef, country, curriculum, grade, level, subject) => {
  try {
    // Assuming you have an endpoint to get lesson data by ref
    const response = await axios.get(`${BASE_URL}/get-lesson-by-ref`, {
      params: {
        lesson_ref: lessonRef,
        country: country,
    ...
    return response.data.data; // Assuming this returns the lesson data
  } catch (error) {
...
const getDynamicStudentId = (): string => {
  const storedId = localStorage.getItem('studentId');

  if (!storedId) {
    console.error('No student ID found. This is a critical error.');
    // Throw an error since a student ID is required
...
const fetchLessonPlan = async (lessonId, studentId) => {
  try {
    // Fetch lesson data using find_lesson_by_ref
    const lessonData = await findLessonByRef(
      lessonId,
      "Nigeria", // Replace with actual country
      "National Curriculum", // Replace with actual curriculum
      "Junior Secondary School 1", // Replace with actual grade
      "JSS1", // Replace with actual level
      "Artificial Intelligence" // Replace with actual subject
    );

    if (!lessonData) {
      throw new Error('Lesson data not found in Firestore');
    }

    // Use lesson data to generate a lesson plan
    const lessonPlanResponse = await localApi.generateLessonPlan({
      lessonRef: lessonId,
      studentId: studentId,
      learningObjectives: lessonData.learningObjectives, // Ensure this is an array
      country: lessonData.country,
      curriculum: lessonData.curriculumType,
      grade: lessonData.gradeLevel,
      level: 'JSS1',
      subject: lessonData.subject,
    });

    if (lessonPlanResponse.success) {
      setLessonPlan(lessonPlanResponse.data.lesson_plan);
    } else {
      throw new Error(lessonPlanResponse.message || 'Failed to generate lesson plan');
    }
  } catch (error) {
    console.error('Error fetching or generating lesson plan:', error);
    setError(error);
  }
};
...
const LessonPage = () => {
  ...
  // Convert learningObjectives to an array if it exists
  const learningObjectivesArray = typeof learningObjectives === 'string'
    ? learningObjectives.split(',').map((objective) => objective.trim())
    : [];
  ...
  }, [isLessonStarted, isLessonPaused]);

  useEffect(() => {
    let interval;
    if (examTimerActive) {
      interval = setInterval(() => {
        setExamTimeLeft((prev) => (prev > 0 ? prev - 1 : 0));
      }, 1000);
    }
    return () => clearInterval(interval);
  }, [examTimerActive]);
  ...
  useEffect(() => {
    if (isVoiceEnabled && lessonData) {
      const toSpeak = new SpeechSynthesisUtterance(
        lessonData.generatedContent.map(c => c.content).join('\n')
      );
      toSpeak.voice = voices.find(v => v.name === 'Google UK English Female') || voices[0];
      toSpeak.rate = 0.9;
      
      synth?.speak(toSpeak);
      setCurrentUtterance(toSpeak);

      return () => synth?.cancel();
    }
  }, [lessonData, isVoiceEnabled, voices]);
  ...
  const handleStartLesson = async () => {
    ...
    if (!studentId || !lessonRef || !country || !curriculum || !grade || !level || !subject || !learningObjectivesArray) {
      setError(new Error('Missing required lesson parameters.'));
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      const initResponse = await localApi.initializeLesson({
        student_id: studentId,
        lesson_ref: lessonRef,
        country: country,
        curriculum: curriculum,
      ...
      setLessonData(initResponse.lessonData);
      setIsLessonStarted(true);

      // Fetch the lesson plan after initializing
      await fetchLessonPlan(lessonRef, studentId);
    } catch (error) {
  ...
  const handleEndLesson = async () => {
    try {
      ...
      if (!notesData?.noteContent) {
        throw new Error('No notes content received');
      }
  
      // Create downloadable file
      const blob = new Blob([notesData.noteContent], { type: 'text/markdown' });
      const url = window.URL.createObjectURL(blob);
      
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `lesson-notes-${lessonData.lessonRef}.md`);
  ...
  const handlePauseResume = async () => {
    if (isLessonPaused) {
      await localApi.resumeLesson(sessionId);
      if (currentUtterance) synth?.resume();
    } else {
      await localApi.pauseLesson(sessionId, "User-initiated pause");
    ...
    return (
      <ContentCard title="Lesson Content">
        <div className="space-y-8">
          {/* Pause/Resume and Voice Control Buttons */}
          <div className="flex justify-end gap-4">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg flex items-center gap-2 ${
                isVoiceEnabled ? 'bg-blue-600' : 'bg-gray-200'
              } text-white`}
            >
        ...
        <div className="min-h-screen bg-gray-100">
          {!isLessonStarted ? (
            <div className="flex items-center justify-center p-6">
              <Button onClick={handleStartLesson} disabled={isLoading}>
                {isLoading ? 'Loading...' : 'Begin Lesson'}
              </Button>
            </div>
          ) : (
